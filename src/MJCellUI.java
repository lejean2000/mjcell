...
class MJCellUI extends Frame
{
  ...
  public  Checkbox chkAdd   = new Checkbox("Add", false);  // Rand/seed add to existing universe?
  ...
  private TextField cmbRandT= new TextField("40%");
  ...
  public void build()
  {
    ...
    pnlLeft.setLayout(new GridLayout(12,1)); // vertical layout for 12 items
    ...
    mnuAnim.add(itmStep);
    mnuAnim.add(itmRewind); itmRewind.setEnabled(false);
    mnuAnim.add("-");
    ...
    setSize(560,430);
    pnlLeft.setVisible(bInitPanelLeft); itmViewControls.setState(bInitPanelLeft);
    pnlBotm.setVisible(bInitPanelBotm); itmViewSeed.setState(bInitPanelBotm);
    add("North", pnlTop);
    add("West", pnlLeft);
    add("South", pnlBotm);
    ...
    mjb.Randomize(cmbRandT.getText(), mjb.RAND_ALL);
  ...
  public void Init()
  {
    ...
    btnRunStop.setFont(new Font(btnRunStop.getFont().getName(), Font.BOLD, btnRunStop.getFont().getSize()));
  }
  // ----------------------------------------------------------------
  ...
  public void InitRules()
  {
    ...
    //when a rule is selected, used defined rules will use its semantics
    mjb.CrrGame = iGame; 
    if (iGame >= 0)
    {
  ...
  public void SendRule(int iGame, String sRuleName, String sRuleDef)
  {
    ...
    if (sRuleDef.length() > 20) sRuleDef = sRuleDef.substring(0,17) + "...";
  ...
  public void processEvent(AWTEvent e)
  {
	if ( e.getID() == Event.WINDOW_DESTROY )
    {
      ...
      PatDlg.setVisible(false);
      PatDlg.removeAll();

      FavDlg.setVisible(false);
      FavDlg.removeAll();
  ...
  public boolean action(Event e, Object arg)
  {
    ...
    else if ((e.target == btnOpen) || (e.target == itmOpen)) // patterns list
    {
      PatDlg.setVisible(true);
      PatDlg.requestFocus();
    }
    else if ((e.target == btnFav) || (e.target == itmFav))// favourite patterns
    {
      FavDlg.setVisible(true);
      FavDlg.requestFocus();
    }
    ...
    {
      mjb.Randomize(cmbRandT.getText(), mjb.RAND_VIEW);
    }
    ...
    else if ((e.target == itmDesc) || (e.target == btnDesc)) // show pattern description
    {
      DialogDesc();
    }
  ...
  {
    // input the user rule
    InputBox ib = new InputBox(new Frame(""), mjb.RuleDef, "User rules", " Enter your own rules (refer to the rules lexicon for syntax):");
    ...
    if (ib.isAccepted)
    {
      ...
      {
        cmbRules.select(MJRules.S_USERRULE); // activate "User rule" item
        SendRule(mjb.CrrGame, MJRules.S_USERRULE, sRuleDef);
        // store the user rule
        mjr.Rules[mjb.CrrGame].addElement(new CARule(MJRules.S_USERRULE, sRuleDef));
      }
  ...
  private void InputCountOfStates()
  {
    ...
    InputBox ib = new InputBox(new Frame(""), sDefault, "Count of states", "Input the count of states (" + sRange + "):");
    requestFocus();
    if (ib.isAccepted)
    {
      int iTmp;
  ...
  private void InputActiveState()
  {
    ...
    InputBox ib = new InputBox(new Frame(""), sDefault, "Active state", "Input the active state (" + sRange + "):");
    requestFocus();
    if (ib.isAccepted)
    {
      int iTmp;
  ...
  {
    String sDefault = String.valueOf(mjb.UnivSize.x) + "x" + String.valueOf(mjb.UnivSize.y);
    String sMax = "max. " + String.valueOf(MJBoard.MAX_X) + "x" + String.valueOf(MJBoard.MAX_Y);
    InputBox ib = new InputBox(new Frame(""), sDefault, "Board size", "Input the new board size (" + sMax + "):");
    requestFocus();
    if (ib.isAccepted)
    {
      ...
      try
      {
        StringTokenizer st = new StringTokenizer(sRetVal, " .,;x-", false);
        if (st.hasMoreTokens())
        {
  ...
  public void UpdateColorsUI()
  {
    ...
    itmCloStatesCnt.setLabel("Count of states... (" + String.valueOf(mjb.StatesCount) + ")");
    itmCloCrrState.setLabel("Active state... (" + String.valueOf(mjb.CrrState) + ")");
    lblStates.setText("States: " + Integer.toString(mjb.CrrState) + "/" + Integer.toString(mjb.StatesCount));
  ...
  public void DialogInfo()
  {
    ...
    ta.append("\n");    ta.append("Board: " + String.valueOf(mjb.UnivSize.x) + "x" + String.valueOf(mjb.UnivSize.y) + "\n");    ta.append("Cell size: " + String.valueOf(mjb.CellSize) + "\n");
    ...
    double dTmp = 100.0 * mjb.Population / (mjb.UnivSize.x*mjb.UnivSize.y);
    dTmp = (Math.round(dTmp * 100.0) / 100.0);
    ta.append("Density: " + Double.toString(dTmp) + "%\n");
    ta.append("\nDistribution:\n");
    for (int i = 0; i < mjb.StatesCount; i++)
    {
      ta.append("State " + Integer.toString(i) + ": " + Integer.toString(mjb.Populations[i]) + "\n");
    }
    msgDlg.add(ta, BorderLayout.CENTER);
    msgDlg.add(btnPnl, BorderLayout.SOUTH);
    btnOk.addActionListener(new ActionListener()      {
        public void actionPerformed(ActionEvent e)        {          msgDlg.dispose();          if (fOldRun) mjb.start();
        }
      }
    );
    msgDlg.addWindowListener(new WindowAdapter()
  ...
  public void setAnimDelay(int newDelay)  {    mjb.setAnimDelay(newDelay);
    btnSlower.setEnabled(mjb.AnimDelay < 1000);    itmSlower.setEnabled(mjb.AnimDelay < 1000);    btnFaster.setEnabled(mjb.AnimDelay > 0);    itmFaster.setEnabled(mjb.AnimDelay > 0);  }
  // ----------------------------------------------------------------
  ...
  public void UpdateUI()
  {
    ...
    lblBoard.setText("Board: " + Integer.toString(mjb.UnivSize.x) + "x" + Integer.toString(mjb.UnivSize.y) + "/" + Integer.toString(mjb.CellSize));
  }
  // ----------------------------------------------------------------
  // applet info
