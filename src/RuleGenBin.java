...
public class RuleGenBin
{
    ...
    for (i = 0; i < sStr.length(); i++)
    {
      ...
      else
      {
        ...
        if ((sStr.charAt(i) == 'a') || (sStr.charAt(i) == 'b'))
        {
          for (j = 0; j < iNum; j++)
          {
            sRetString = sRetString + ((sStr.charAt(i) == 'a') ? "0" : "1");
          }
          iNum = 0;
        }
      }
  ...
  public void InitFromPrm(int i_Clo, boolean is_Hist, int i_Ngh, boolean rules_S[], boolean rules_B[])
  {
      ...
      for (j = 0; j < sizY; ++j)
      {
        ...
        if (isHist) // with history
        {
          if (bOldVal <= 1) // can survive or be born
          {
            ...
            {
  	          if (crrState[i][lurd[1]]       == 1) iCnt += 1;
  	          if (crrState[lurd[2]][lurd[1]] == 1) iCnt += 2;
  	          if (crrState[lurd[2]][j]       == 1) iCnt += 4;
  	          if (crrState[lurd[2]][lurd[3]] == 1) iCnt += 8;
  	          if (crrState[i][lurd[3]]       == 1) iCnt += 16;
   	          if (crrState[lurd[0]][lurd[3]] == 1) iCnt += 32;
  	          if (crrState[lurd[0]][j]       == 1) iCnt += 64;
              if (crrState[lurd[0]][lurd[1]] == 1) iCnt += 128;
            }
            else
            {
  	          if (crrState[i][lurd[1]]       == 1) iCnt += 1;
  	          if (crrState[lurd[2]][j]       == 1) iCnt += 2;
  	          if (crrState[i][lurd[3]]       == 1) iCnt += 4;
  	          if (crrState[lurd[0]][j]       == 1) iCnt += 8;
            };
        ...
        else // no history
        {
          if (iNgh == MJRules.NGHTYP_MOOR)
          {
  	        if (crrState[i][lurd[1]]       != 0) iCnt += 1;
  	        if (crrState[lurd[2]][lurd[1]] != 0) iCnt += 2;
  	        if (crrState[lurd[2]][j]       != 0) iCnt += 4;
  	        if (crrState[lurd[2]][lurd[3]] != 0) iCnt += 8;
  	        if (crrState[i][lurd[3]]       != 0) iCnt += 16;
   	        if (crrState[lurd[0]][lurd[3]] != 0) iCnt += 32;
  	        if (crrState[lurd[0]][j]       != 0) iCnt += 64;
            if (crrState[lurd[0]][lurd[1]] != 0) iCnt += 128;
          }
          else
          {
  	        if (crrState[i][lurd[1]]       != 0) iCnt += 1;
  	        if (crrState[lurd[2]][j]       != 0) iCnt += 2;
  	        if (crrState[i][lurd[3]]       != 0) iCnt += 4;
  	        if (crrState[lurd[0]][j]       != 0) iCnt += 8;
          };
