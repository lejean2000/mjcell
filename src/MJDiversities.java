...
public class MJDiversities
{
  ...
  public void ItemFromString(String sStr, int sizX, int sizY)
  {
    ...
    try
    {
           if (sStr.startsWith("#SYSTEM")) itm = DIV_SYSTEM;
      else if (sStr.startsWith("#NOISE")) itm = DIV_NOISE;
      else if (sStr.startsWith("#BHOLE")) itm = DIV_BHOLE;
      else if (sStr.startsWith("#SNOVA")) itm = DIV_SNOVA;
      else if (sStr.startsWith("#STRIN")) itm = DIV_STRIN;
      ...
      if (itm != DIV_NONE)
      {
        ...
        while (st.hasMoreTokens())
        {
          ...
          {
            case DIV_SYSTEM:
                          if (sTok.startsWith("ACT="))                            m_Enabled = Integer.valueOf(sTok.substring(4)).intValue() != 0;
                          break;
            case DIV_NOISE:
                          if (sTok.startsWith("ACT="))                            m_NoiseActive = Integer.valueOf(sTok.substring(4)).intValue() != 0;                          else if (sTok.startsWith("CYCL="))                            m_NoiseCycles = Integer.valueOf(sTok.substring(5)).intValue();                          else if (sTok.startsWith("CELL="))                            m_NoiseCells = Integer.valueOf(sTok.substring(5)).intValue();                          else if (sTok.startsWith("STT="))                            m_NoiseState = Integer.valueOf(sTok.substring(4)).intValue();                          break;
            case DIV_BHOLE:
                          if (sTok.startsWith("ACT="))                            m_BHoleActive = Integer.valueOf(sTok.substring(4)).intValue() != 0;
                          else if (sTok.startsWith("X="))
                          {                            m_BHoleCtrX = Integer.valueOf(sTok.substring(2)).intValue();                            m_BHoleCtrX += sizX / 2;
                          }                          else if (sTok.startsWith("Y="))
                          {                            m_BHoleCtrY = Integer.valueOf(sTok.substring(2)).intValue();
                            m_BHoleCtrY += sizY / 2;
                          }                          else if (sTok.startsWith("SIZE="))                            m_BHoleSize = Integer.valueOf(sTok.substring(5)).intValue();                          break;
            case DIV_SNOVA:
                          if (sTok.startsWith("ACT="))                            m_SNovaActive = Integer.valueOf(sTok.substring(4)).intValue() != 0;
                          else if (sTok.startsWith("X="))
                          {                            m_SNovaCtrX = Integer.valueOf(sTok.substring(2)).intValue();
                            m_SNovaCtrX += sizX / 2;
                          }                          else if (sTok.startsWith("Y="))                          {
                            m_SNovaCtrY = Integer.valueOf(sTok.substring(2)).intValue();
                            m_SNovaCtrY += sizY / 2;
                          }                          else if (sTok.startsWith("SIZE="))                            m_SNovaSize = Integer.valueOf(sTok.substring(5)).intValue();                          else if (sTok.startsWith("STT="))                            m_SNovaState = Integer.valueOf(sTok.substring(4)).intValue();                          break;
            case DIV_STRIN:
                          {
  ...
  public boolean ItemActive(int itm)
  {
    ...
    switch (itm)
    {
      case DIV_SYSTEM: bRet = m_Enabled;      break;
      case DIV_NOISE:  bRet = m_NoiseActive;  break;
      case DIV_BHOLE:  bRet = m_BHoleActive;  break;
      case DIV_SNOVA:  bRet = m_SNovaActive;  break;
      case DIV_STRIN:  bRet = StrIn.m_Active; break;
    }
    return (bRet);
  }
  //------------------------------------------------------------------------------
  ...
  public void Perform(boolean BeforePass, MJBoard mjb)
  {
    ...
    // noise active?
    if (m_NoiseActive && BeforePass && (m_NoiseCycles > 0) && (m_NoiseCells > 0))
    {
      if ((mjb.Cycle % m_NoiseCycles) == 0)
      {
        ...
        {
          iCol = (int)Math.ceil(Math.random() * (mjb.UnivSize.x - 1));
          iRow = (int)Math.ceil(Math.random() * (mjb.UnivSize.y - 1));
    ...
    if (StrIn.m_Active && BeforePass)
    {
      ...
      if (iSiz > 0) // any items exist?
      {
        ...
        mjb.SetCell(StrIn.m_X, StrIn.m_Y, ((Integer)(StrIn.m_Vals.elementAt(StrIn.m_Pos))).shortValue());
        StrIn.m_Pos++;
      }
    }
