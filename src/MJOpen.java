...
public class MJOpen
{
  ...
  private Vector vCells; // cells found in the pattern file
  private Vector vDescr; // pattern description
  private Vector vDiv;   // diversities

  private String m_sGame;    // family of rules
  private String m_sRules;   // the rule
  private int    rectMinX, rectMaxX, rectMinY, rectMaxY;
  ...
  public boolean OpenFile(String sFileName)
  {
    ...
    vCells = new Vector();
    vLines = new Vector();
    vDescr = new Vector(); // description
    vDiv   = new Vector(); // diversities
...
//System.out.println(sFilePath);
    ...
    if (mjT.LoadTextFile(sFilePath, vLines)) // load the file into the vector
    {
      if (!vLines.isEmpty()) // was anything read?
      {
        ...
        if (!fOk) fOk = ReadLife105(vLines);
        if (!fOk) fOk = ReadLife106(vLines);
        if (!fOk) fOk = ReadMCL(vLines);
        if (!fOk) fOk = ReadRLE(vLines);

        if (fOk) // pattern loaded
        {
          if (vCells.size() > 0) // any cells found
            AddPattern();
        }
        else
        {
          System.out.println("Unrecognized file format: " + sFilePath);
        }
      }
      else
      {
  ...
  private void AddPattern()
  {
    ...
    CalcMinRectangle();
    sizeX = rectMaxX - rectMinX;
    sizeY = rectMaxY - rectMinY;
    ...
    for (i = 0; i < vDiv.size(); i++)
      mjb.Div.ItemFromString((String)vDiv.elementAt(i), mjb.UnivSize.x, mjb.UnivSize.y);

    // add all cells
    mjb.Clear(false);
    dx = mjb.UnivSize.x / 2 - (rectMaxX + rectMinX) / 2 - 1;
    if (mjb.GameType == MJRules.GAMTYP_2D)
      dy = mjb.UnivSize.y / 2 - (rectMaxY + rectMinY) / 2 - 1;
    else
      dy = 0;
    for (i = 0; i < vCells.size(); i++)
    {
      cell = (CACell)vCells.elementAt(i);
      x = cell.x + dx;
      y = cell.y + dy;
  ...
  {
    CACell cell = new CACell();
    rectMinX = rectMinY =  999999;
    rectMaxX = rectMaxY = -999999;

    for (int i = 0; i < vCells.size(); i++)
    {
      cell = (CACell)vCells.elementAt(i);

      if (rectMinX > cell.x) rectMinX = cell.x;
      if (rectMaxX < cell.x) rectMaxX = cell.x;
      if (rectMinY > cell.y) rectMinY = cell.y;
      if (rectMaxY < cell.y) rectMaxY = cell.y;
    }
  }
  // ----------------------------------------------------------------
  // ----------------------------------------------------------------
  ...
  boolean ProcessOneLIF105Line(String bff)
  {
    ...
    {
      // special characters?
      if ((bff.charAt(0) == '#') || (bff.charAt(0) == '!') || (bff.charAt(0) == '/'))
      {
        if (bff.startsWith("#P")) // the block position
        {
          ...
          st.nextToken();  // #P
          iRow105 = 0;
          if (st.hasMoreTokens()) iBlkX = Integer.parseInt(st.nextToken());
          if (st.hasMoreTokens()) iBlkY = Integer.parseInt(st.nextToken());
        }
        ...
        else if (bff.startsWith("#R")) // specific rules
        {
          StringTokenizer st = new StringTokenizer(bff);
          st.nextToken();  // #R
          if (st.hasMoreTokens()) m_sRules = st.nextToken();
        }
        else if (bff.startsWith("#S")) // speed
        {
          StringTokenizer st = new StringTokenizer(bff);
          st.nextToken();  // #S
          if (st.hasMoreTokens()) m_Speed = Integer.parseInt(st.nextToken());
        }
        else if (bff.startsWith("#D") || bff.startsWith("#C")) // description
        {
          ...
          vDescr.addElement(sTok); // add the comment line
        }
      ...
      else // cells data line
      {
        ...
        for (i = 0; i < bff.length(); i++)
        {
          ...
          }          else
          {
            if (iNum == 0) iNum = 1;
             // alive cell?
            if ((bff.charAt(i) == '*') || (bff.charAt(i) == 'o') || (bff.charAt(i) == 'O'))
            {
              for (j = 0; j <= iNum-1; j++)
                vCells.addElement(new CACell(iCol+j+iBlkX, iRow105+iBlkY, (short)1));
              fOk = true;
              iCol = iCol + iNum;
            }
            else // blank
        ...
        for (i = 0; i < bff.length(); i++)
        {
          ...
          else
          {
            ...
            switch (bff.charAt(i))
            {
              ...
              default:
                   if ((bff.charAt(i) >= 'a') && (bff.charAt(i) <= 'j'))
                   {
                     // a: 25..48, b: 49..72, c: 73..96, ..., j: 241..255
                     iAdd = (bff.charAt(i) - 'a' + 1) * 24;
                   }
                   else if ((bff.charAt(i) >= 'A') && (bff.charAt(i) <= 'X'))
                   {
                     // cell in state 1..24
                     for (j = 0; j < iNumMCL; j++)
                     {
                       vCells.addElement(new CACell(iColMCL+j, iRowMCL, (short)(bff.charAt(i) - 'A' + 1 + iAdd)));
                     }
                     iColMCL = iColMCL + iNumMCL;
                     fOk = true; // any cell added
                     iAdd = 0;
  ...
  private boolean ProcessOneRLELine(String bff)
  {
    ...
    else
    {
      ...
      {
        vDescr.addElement(bff); // add the comment line
      }
      else
      {
        if (bff.length() > 0)
          ...
          if ((!fXYFound) && bff.startsWith("x")) // the first line
          {
            ...
            while (stcomma.hasMoreTokens())
            {
              ...
              if (tokenType.equals("x")) // X size
                iniCol = iCol = -(Math.abs(Integer.parseInt(tokenValue)) / 2);
              ...
              }              else
              {
                ...
                switch (bff.charAt(i))
                {
                  ...
                  default: // probably a cell
                         if (    ((bff.charAt(i) >= 'a') && (bff.charAt(i) <= 'z'))
                              || ((bff.charAt(i) >= 'A') && (bff.charAt(i) <= 'Z'))
                            )
                         {
                           switch (bff.charAt(i))
                           {
                           ...
                           for (j = 0; j <= iNum-1; j++)
                             vCells.addElement(new CACell(iCol+j, iRow, (short)iTmp));
      ...
      else // a cell
      {
        ...
        if (st.hasMoreTokens())
        {
          ...
          if (st.hasMoreTokens())
          {
            ...
            vCells.addElement(new CACell(iCol, iRow, (short)1));
            fOk = true;
          }
        }
      }
