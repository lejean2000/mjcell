...
{
  public  static final int MAX_X = 2000;  // Universe limits
  ...
  public  int       Populations[] = new int[MAX_CLO+1]; // count of cells in each state
  private int       lastX = 0, lastY = 0; // mouse handling
  public  boolean   IsRunning = false; // is running?
  ...
  // misc
  public  MJDiversities Div;      // diversities system (black hole, nova, injection, etc.)
  ...
  public void SingleStep()
  {
    ...
    else // pagewise
    {
      ...
      if (iCnt <= 0) iCnt = UnivSize.y; // the whole page
    }
  ...
  int OneCycle()
  {
    ...
    try
    {
      ...
      switch (CrrGame)
      {
        case MJRules.GAME_LIFE: // Standard Conway-like game
             modCnt = RLife.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState, this);
             break;

        case MJRules.GAME_VOTE: // Vote for life
             modCnt = RVote.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState, this);
             break;

        case MJRules.GAME_WLIF: // Weighted life
             modCnt = RWLife.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState, this);
             break;

        case MJRules.GAME_GENE: // Generations
             modCnt = RGene.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState, this);
             break;
        ...
        case MJRules.GAME_CYCL: // Cyclic CA
             modCnt = RCyclic.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState);
             break;
        ...
        case MJRules.GAME_1DBI: // 1D binary
             modCnt = R1DBin.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState, this);
             break;

        case MJRules.GAME_NMBI: // Neumann binary
             modCnt = RNeumBin.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState);
             break;

	      case MJRules.GAME_GEBI: // General binary
             modCnt = RGenBin.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState, this);
             break;

	      case MJRules.GAME_LGTL: // Larger than Life
             modCnt = RLgtL.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState, this);
             break;

	      case MJRules.GAME_MARG: // Margolus
             modCnt = RMarg.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState, this);
             break;

        case MJRules.GAME_USER: // User DLL
             modCnt = RUser.OnePass(UnivSize.x, UnivSize.y, WrapAtEdges, ColoringMethod, crrState, tmpState);
             break;
      }
    ...
    if (modCnt > 0) Cycle++; // next cycle performed
  ...
  final public void SetCell(int x, int y, short bState)
  {
    if ((x >= 0) && (y >= 0) && (x < UnivSize.x) && (y < UnivSize.y))
    {
      if (crrState[x][y] != bState)
      {
        ...
        Populations[bState]++;
        if (0 == crrState[x][y]) Population++;
        else if (0 == bState) Population--;
        crrState[x][y] = bState;
      }
    }
  }
  ...
  {
    if ((cell.x >= 0) && (cell.y >= 0) && (cell.x < UnivSize.x) && (cell.y < UnivSize.y))
    {
      if (crrState[cell.x][cell.y] != cell.state)
      {
        Populations[crrState[cell.x][cell.y]]--;
        Populations[cell.state]++;
        if (0 == crrState[cell.x][cell.y]) Population++;
        else if (0 == cell.state) Population--;
  ...
  public void InitBoard(int sizX, int sizY, int cellSiz)
  {
    ...
    if (sizX > MAX_X) sizX = MAX_X;
    if (sizY > MAX_Y) sizY = MAX_Y;
    if (sizX < 10) sizX = 10;
    if (sizY < 10) sizY = 10;
    // stop for a while
    stop();
    ...
    if (RecalcLayout()) // recalc all board basic parameters
    {
      ...
      offSrs = new MemoryImageSource(ViewSize.x, ViewSize.y, screen, 0, ViewSize.x);
      ...
      setVisible(false); setVisible(true);  // only this trick repaints the panel
  ...
  public boolean RecalcLayout()
  {
    ...
    pnlBotm.setBounds(0, hgt-sbarWidth, wdt, sbarWidth);
    btnZoomIn.setBounds(0, 0, sbarWidth, sbarWidth);
    btnZoomOut.setBounds(sbarWidth+1, 0, sbarWidth, sbarWidth);
    iTmp = btnZoomIn.getSize().width+btnZoomOut.getSize().width;
    hSbar.setBounds(iTmp+1, 0, wdt-sbarWidth-iTmp-1, sbarWidth);
    btnFit.setBounds(wdt-sbarWidth, 0, sbarWidth, sbarWidth);
    vSbar.setBounds(wdt-sbarWidth, 0, sbarWidth, hgt-sbarWidth);
    ...
    if ((wdt > CellSize) && (hgt > CellSize))
    {
      ...
      //System.out.println("ViewSize: " + String.valueOf(ViewSize.x) + "x" + String.valueOf(ViewSize.y));
      //System.out.println("CellsInView: " + String.valueOf(CellsInView.x) + "x" + String.valueOf(CellsInView.y));
    }
  ...
  {
    if (ViewOrg.x > UnivSize.x-CellsInView.x) ViewOrg.x = UnivSize.x-CellsInView.x;
    if (ViewOrg.y > UnivSize.y-CellsInView.y) ViewOrg.y = UnivSize.y-CellsInView.y;
    if (ViewOrg.x < 0) ViewOrg.x = 0;
    if (ViewOrg.y < 0) ViewOrg.y = 0;
  }
  // ----------------------------------------------------------------
  // center the board
  ...
  public Rectangle CalcPatternRect()
  {
    ...
    for (iCol = 0; iCol < UnivSize.x; iCol++)
      for (iRow = 0; iRow < UnivSize.y; iRow++)
        ...
        {
          if (rct.x > iCol) rct.x = iCol;
          if (rct.x + rct.width - 1 < iCol) rct.width = iCol - rct.x + 1;
          if (rct.y > iRow) rct.y    = iRow;
          if (rct.y + rct.height - 1 < iRow) rct.height = iRow - rct.y + 1;
        }
    return rct;
  }
  // ----------------------------------------------------------------
  ...
  public void CenterPoint(int ix, int iy, boolean fRedraw)
  {
    ...
    CellsInView.x = (LastPanelSize.x - sbarWidth - 1) / CellSize; // vsb. cells count
  ...
  public void Fit(boolean fRedraw)
  {
    ...
    if (rct.width >= 0) // any cell exists
    {
      ...
      // select the biggest size that covers the full pattern
           if (fac >= 12) iFac = 11;
      else if (fac >= 10) iFac = 9;
      else if (fac >=  7) iFac = 7;
      else if (fac >=  5) iFac = 5;
      else if (fac >=  4) iFac = 4;
      else if (fac >=  3) iFac = 3;
      else if (fac >=  2) iFac = 2;
      else                iFac = 1;
  ...
  {
    if ((LastPanelSize.x != this.getSize().width) || (LastPanelSize.y != this.getSize().height))
    {
      InitBoard(UnivSize.x, UnivSize.y, CellSize); // resized, update board parameters
    }
    if (ViewSize.x > 0)
    {
      offSrs.newPixels();
  ...
  public void RedrawBoard(boolean fDrawAll)
  {
    ...
    try
    {
    	for (i = 0; i < CellsInView.x; i++)
      {
        ...
        {
          newClo = mjPal.Palette[crrState[ViewOrg.x + i][ViewOrg.y + j]];
          // now paint the cells - set proper 'screen' array fields
          if ((fDrawAll) || (screen[(j*CellSize)*dx + i*CellSize] != newClo))
          {
            switch (CellSize)
            {
              case 1:
                      ...
                      break;
              case 2:
                      screen[iTmpCol = (j*CellSize+0)*dx + ixCellSize] = newClo;
                      screen[++iTmpCol] = newClo;
                      screen[iTmpCol = (j*CellSize+1)*dx + ixCellSize] = newClo;
                      screen[++iTmpCol] = newClo;
                      break;
              default:
                      for (ic = 0; ic < CellSize; ic++)
                      {
                        ...
                        {
                          if (fDrawGrid && ((ic == CellSize-1) || (jc == CellSize-1)))
                          {
                            if (    ((ic == CellSize-1) && ((ViewOrg.y + j) % 5 == 0))
                                 || ((jc == CellSize-1) && ((ViewOrg.x + i) % 5 == 0))
                               )
                              screen[iTmpCol++] = mjPal.GridColor[1]; // bold grid line
                            else
                              screen[iTmpCol++] = mjPal.GridColor[0]; // normal grid line
                          }
                          else
                          {
                            screen[iTmpCol++] = newClo;
  ...
  public void SetRule(int iGame, String sRuleNam, String sRuleDef)
  {
    ...
    switch (CrrGame)
    {
      ...
      case MJRules.GAME_WLIF: // Weighted life
           ...
           if (RWLife.isHist) SetStatesCount(RWLife.iClo);
           break;
      ...
      case MJRules.GAME_1DTO: // 1D totalistic
           ...
           if (R1DTo.isHist) SetStatesCount(R1DTo.iClo);
           break;
  ...
  {
    if ((p2 >= OfsX) && (p3 >= OfsY) &&
        (p2 <= OfsX + UnivSize.x*CellSize) && (p3 <= OfsY + UnivSize.y*CellSize))
    {
  ...
  public void DrawLine(int x1, int y1, int x2, int y2)
  {
    ...
    if (x2 > x1) xRight = 1; else xRight = -1;
    if (y2 > y1) yDown  = 1; else yDown  = -1;
  ...
  public void Seed(String sHow)
  {
    ...
    if (sHow.startsWith("BLK")) // block of cells
    {
      ...
      if (iPos >= 0)
      {
        ...
        int dy = Integer.valueOf(sHow.substring(iPos+1, sHow.length())).intValue();
        if (GameType == MJRules.GAMTYP_2D)
        {
          for (i = 0; i < dx; i++)
            for (j = 0; j < dy; j++)
              SetCell(ctrX-(dx/2)+i, ctrY-(dy/2)+j, (short)CrrState); // active state (color)
        }
        else // 1D
        {
          for (i = 0; i < dx; i++)
            SetCell(ctrX-(dx/2)+i, i1DLastRow, (short)CrrState); // active state (color)
        }
      }
    }
    else if (sHow.startsWith("FRM")) // frame of cells
    {
      ...
      if (iPos >= 0)
      {
        ...
        if (GameType == MJRules.GAMTYP_2D)
        {
          for (i = 0; i < dx; i++)
          {
            SetCell(ctrX-(dx/2)+i, ctrY-(dy/2), (short)CrrState); // active state (color)
            SetCell(ctrX-(dx/2)+i, ctrY-(dy/2)+dy-1, (short)CrrState); // active state (color)
          }
          for (j = 1; j < dy-1; j++)
          {
            SetCell(ctrX-(dx/2), ctrY-(dy/2)+j, (short)CrrState); // active state (color)
            SetCell(ctrX-(dx/2)+dx-1, ctrY-(dy/2)+j, (short)CrrState); // active state (color)
          }
        }
        else // 1D
        {
          SetCell(ctrX-(dx/2), i1DLastRow, (short)CrrState); // active state (color)
          SetCell(ctrX+(dx/2)-1, i1DLastRow, (short)CrrState); // active state (color)
        }
      }
    }
