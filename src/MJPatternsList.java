...
public class MJPatternsList extends Dialog implements ActionListener
{
  ...
  private Vector<String>   vPatterns[] = (Vector<String>[])new Vector[MJRules.GAME_LAST + 1];
  ...
  private void AddPatterns()
  {
    ...
    if (mjT.LoadResTextFile("pat.txt", vLines)) // load the file with pattern names
    {
      for (i = 0; i < vLines.size(); i++)
      {
        ...
        if ((sBff.length() > 0) && !((String)vLines.elementAt(i)).startsWith("//"))
        {
  ...
  public void InitList()
  {
    ...
    if (mjUI.mjr.IsGameIdxValid(iGame))
      for (i = 0; i < vPatterns[iGame].size(); i++)
        if (((String)vPatterns[iGame].elementAt(i)).startsWith(sRuleName + "/"))
          LstFiles.add(((String)vPatterns[iGame].elementAt(i)).substring(sRuleName.length()+1));
  }
  // ----------------------------------------------------------------
