...
public class RuleNeumBin
{
  ...
  public void InitFromString(String sStr)
  {
    ...
    iPos++;
    if (iClo < 2) iClo = 2;
    if (iClo > MAX_STATES) iClo = MAX_STATES;
        ...
        for (int k = 0; k < iClo; k++)
          for (int l = 0; l < iClo; l++)
            for (int m = 0; m < iClo; m++)
            {
              ...
              if (iStt < 0) iStt = 0;
              if (iStt >= MAX_STATES) iStt = MAX_STATES - 1;
  ...
  {
    if (iClo < 2) iClo = 2;
    else if (iClo > MAX_STATES) iClo = MAX_STATES;
  }
  // ----------------------------------------------------------------
  ...
  public int OnePass(int sizX, int sizY, boolean isWrap, int ColoringMethod, short crrState[][], short tmpState[][])
  {
...
    for (i = 0; i < sizX; ++i)
    {
...
      for (j = 0; j < sizY; ++j)
      {
        ...
        bOldVal = (short)((bOldVal < 0) ? 0 : (bOldVal >= iClo) ? (iClo - 1) : bOldVal);
