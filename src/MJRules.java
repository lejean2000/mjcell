...
public class MJRules
{
...
  public Vector<Object> Rules[] = new Vector[GAME_LAST+1]; // array of rules
  ...
  private void AddRules()
  {
    ...
    if (mjT.LoadResTextFile("rul.txt", vLines)) // load the file with rules
    {
      for (i = 0; i < vLines.size(); i++)
      {
        ...
        if ((sBff.length() > 0) && !(vLines.elementAt(i)).startsWith("//"))
        {
  ...
  public String GetGameName(int iGame)
  {
    ...
    switch (iGame)
    {
      case GAME_LIFE: sRetVal = GAME_LIFE_Name; break; // Life
      case GAME_GENE: sRetVal = GAME_GENE_Name; break; // Generations
      case GAME_WLIF: sRetVal = GAME_WLIF_Name; break; // Weighted life
      case GAME_VOTE: sRetVal = GAME_VOTE_Name; break; // Vote for life
      case GAME_RTBL: sRetVal = GAME_RTBL_Name; break; // Rules table
      case GAME_CYCL: sRetVal = GAME_CYCL_Name; break; // Cyclic CA
      case GAME_1DTO: sRetVal = GAME_1DTO_Name; break; // 1D totalistic
      case GAME_1DBI: sRetVal = GAME_1DBI_Name; break; // 1D binary
      case GAME_NMBI: sRetVal = GAME_NMBI_Name; break; // Neumann binary
	  case GAME_GEBI: sRetVal = GAME_GEBI_Name; break; // General binary
  	  case GAME_LGTL: sRetVal = GAME_LGTL_Name; break; // Larger than Life
  	  case GAME_MARG: sRetVal = GAME_MARG_Name; break; // Margolus
      case GAME_USER: sRetVal = GAME_USER_Name; break; // User DLL
      case GAME_SPEC: sRetVal = GAME_SPEC_Name; break; // Special rules
  ...
  public String GetRuleDef(String sGameName, String sRuleName)
  {
    ...
    iGame = GetGameIndex(sGameName);    if (iGame >= 0)    {      for (i = 0; i < Rules[iGame].size(); i++)
      {
        if (sRuleName.compareTo(((CARule)Rules[iGame].elementAt(i)).name) == 0)
        {
  ...
  public String GetRuleName(String sGameName, String sRuleDef)
  {
    ...
    if (iGame >= 0)    {
      for (i = 0; i < Rules[iGame].size(); i++)
      {        if (sRuleDef.compareTo(((CARule)Rules[iGame].elementAt(i)).def) == 0)
        {
