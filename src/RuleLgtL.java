...
public class RuleLgtL
{
  ...
  public void ResetToDefaults()
  {
    ...
    iSMin   = 34; iSMax = 58; // surviving rules
    iBMin   = 34; iBMax = 45; // birth rules
    isCentr = true;           // use the center (middle) cell?
  }
  // ----------------------------------------------------------------
  // Parse the rule string
  // Example: "R5,C0,M1,S34..58,B34..45,NM"
    ...
    while (st.hasMoreTokens())
    {
      ...
      else if (sTok.startsWith("B")) // birth rules
      {
        if (sTok.length() >= 4)
        {
          ...
          if (iTmp >= 0)
          {
            iBMin = Integer.valueOf(sTok.substring(1, iTmp)).intValue();
            iBMax = Integer.valueOf(sTok.substring(iTmp + 2)).intValue();
          }
        }
      }
    }
                          ...
                          int i_SMin, int i_SMax, int i_BMin, int i_BMax, boolean is_Centr)
  {
    ...
    iSMin   = i_SMin; iSMax = i_SMax; // surviving rules
    iBMin   = i_BMin; iBMax = i_BMax; // birth rules
  ...
  public String GetAsString()
  {
    ...
    // S rules
    sBff = sBff + ",S" + String.valueOf(iSMin) + ".." + String.valueOf(iSMax);

    // B rules
    sBff = sBff + ",B" + String.valueOf(iBMin) + ".." + String.valueOf(iBMax);
  ...
  public int OnePass(int sizX, int sizY, boolean isWrap, int ColoringMethod, short crrState[][], short tmpState[][], MJBoard mjb)
  {
    ...
    int   modCnt = 0;
    int   i, j, iCnt;
    int   xVector[] = new int[2*MAX_RANGE+1]; 
    int   yVector[] = new int[2*MAX_RANGE+1]; 
    ...
    for (i = 0; i < sizX; i++)
    {
      for (j = 0; j < sizY; j++)
      {
        ...
        {
          colL = i - iTmp;
          if (colL >= 0)   xVector[MAX_RANGE-iTmp] = colL;
          else             xVector[MAX_RANGE-iTmp] = sizX + colL;

          colR = i + iTmp;
          if (colR < sizX) xVector[MAX_RANGE+iTmp] = colR;
          else             xVector[MAX_RANGE+iTmp] = colR - sizX;

          rowT = j - iTmp;
          if (rowT >= 0)   yVector[MAX_RANGE-iTmp] = rowT;
          else             yVector[MAX_RANGE-iTmp] = sizY + rowT;

          rowB = j + iTmp;
          if (rowB < sizY) yVector[MAX_RANGE+iTmp] = rowB;
          else             yVector[MAX_RANGE+iTmp] = rowB - sizY;
        }
        ...
        if (isHist)
        {
          ...
          {
            for (ic = MAX_RANGE - iRng;  ic <= MAX_RANGE + iRng; ic++)
            {
              for (ir = MAX_RANGE - iRng;  ir <= MAX_RANGE + iRng; ir++)
              {
                if ((isCentr) || (ic != i) || (ir != j))
                {
                  if ((fMoore) || ((Math.abs(ic - MAX_RANGE) + Math.abs(ir - MAX_RANGE)) <= iRng))
                  {
