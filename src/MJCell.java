...
/*
0.67, 20.11.1999
- New rules: Sedimental (Generations), Conway--, Conway++, Conway+-1,
  Conway+-2, Fleas2, NoFleas2, Hexrule b2o, Starbursts2 (Weighted Life)
...
1.02, 19.03.2000
- Many new rules and patterns in 1D Totalistic, CyclicCA, Generations,
  Larger than Life, Neumann binary, and Weighted Life.

1.10, 26.05.2000
- Possibility to run the applet also as a Java applivation (with Java.exe).
...
- Rules definitions were extracted from the compiled applet
  and are now specified in an external text file. It's no longer
  necessary to recompile the applet just in order to add a new rule.
- New rules:
  Generations: Burst, BurstII, Circuit_Genesis, BelZhab Sediment, FlamingStarbows,
  Glisserati, Rake, Snake
  Cyclic CA: Cubism, LavaLamp, LowBrowFour
  Larger that Life: ModernArt
  Life: InverseLife, WalledCities
...
public class MJCell extends Applet
{
  ...
  private static final String VsnNum = "1.50";
  ...
  public void init()
  {
    ...
    if (isApplet)
    {
      ...
      if (sBtnLabel == null) sBtnLabel = "Start MJCell";
      btnStart.setLabel("  " + sBtnLabel + "  ");

      // background
      String sColor = getParameter("Background");
      if (sColor != null) setBackground(Color.decode(sColor));

      // family
      String sFamily = getParameter("Family");
      if (sFamily != null) mjcUI.sInitGame = sFamily;

      // rule
      String sRule = getParameter("Rule");
      if (sRule != null) mjcUI.sInitRule = sRule;

      // pattern
      String sPattern = getParameter("Pattern");
      if (sPattern != null) mjcUI.sInitPatt = sPattern;

      // UI items visibility
      String sInitPanelLeft = getParameter("ViewPanelControls");
      if (sInitPanelLeft != null) mjcUI.bInitPanelLeft = sInitPanelLeft.compareTo("1") == 0;

      String sInitPanelBotm = getParameter("ViewPanelSeeding");
      if (sInitPanelBotm != null) mjcUI.bInitPanelBotm = sInitPanelBotm.compareTo("1") == 0;
    }
    else
    {
  ...
  public String getAppletInfo()
  {
    return getAppletName() + "\n\n" + 
           "Copyright (C) Mirek Wojtowicz, 1999, 2000\n" + 
           "e-mail: mirwoj@homemail.com\n" + 
           "http://www.mirwoj.opus.chelm.pl\n\n" +
